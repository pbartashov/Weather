...
public final class CoreDataContextProvider {
    ...
    public init(completionClosure: ((Error?) -> Void)? = nil) {
        ...
        guard let bundle = Bundle(identifier: "pB.WeatherKit"),
              let url = bundle.url(forResource: "Weather", withExtension: "momd"),
              let model = NSManagedObjectModel(contentsOf: url)
        else {
            fatalError("Failed to load Core Data model")
        }

        persistentContainer = NSPersistentContainer(name: "Weather", managedObjectModel: model)
